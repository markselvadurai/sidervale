...
export type CloudCoverResult =
  | { available: true; avgCloud: number; coverage: number }
  | { available: false };
...
export async function getForecast(site: Site) : Promise<Forecast> {
    ...
    for (let i = 0; i < data.hourly.time.length; i++) {
        hours.push({time: DateTime.fromISO(data.hourly.time[i], { zone: 'utc' }).setZone(site.timezone), cloudCover: data.hourly.cloud_cover[i]});
    }
