...
Sidervale computes the true-darkness window, moon interference, and cloud cover for curated dark-sky destinations, fuses them into a single 0–100 score, and answers the only question that matters before a three-hour drive: _go, or wait for Thursday._
...
Pure functions. No Angular, no DOM, fully unit-tested — which is what lets the same code run in the browser _and_ server-side in a precompute pipeline.
